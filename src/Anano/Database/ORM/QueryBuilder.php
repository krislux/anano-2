...
class QueryBuilder extends \Anano\Database\Database
{
     ...
     */
    public function buildSelect($partial = false)
    {
        $parts = array();
        $placeholders = array();
        ...
        if ( ! empty($this->query['whereGroup'])) {
            if ( ! in_array('WHERE', $parts)) {
                $parts[] = 'WHERE';
            }

            foreach ($this->query['whereGroup'] as $group) {
                $parts[] = '('.str_replace('WHERE ', '', $group['query']).')';

                foreach ($group['placeholders'] as $p) {
                    $placeholders[] = $p;
                }
            }
        }
     ...
     */
    private function buildInsert(array $fields, $replace = false, $ignore = false)
    {
        $parts = array();
        $placeholders = array();
        ...
        if ($replace) {
            ...
            $parts[] = 'INSERT '.($ignore ? 'IGNORE ' : '').'INTO';
        }
    ...
    {
        $set = $this->select('COUNT(*)')->get()->next();
    ...
    public function insert(array $fields, $ignore = false)
    {
        $build = $this->buildInsert($fields, false, $ignore);
        $this->clearQuery();
    ...
    public function whereIn($var1, array $list)
    {
        $list = implode(',', $list);
        $this->query['where'][] = array($var1, " IN ($list)", false);
    }

    public function whereNotIn($var1, array $list)
    {
        $list = implode(',', $list);
        $this->query['where'][] = array($var1, " NOT IN ($list)", false);
    }
    ...
    public function group(callable $callback)
    {
        $group = new self();
        $callback($group);
        $this->query['whereGroup'][] = $group->buildSelect(true);
